...
# ---- Create a meaningful save directory with training params ----
...
if embedding_type == 'glove':
    save_dir_name = f"glove_{embedding_dim}e{epochs}bs{batch_size}lr{lr}"
elif embedding_type == 'sentence_transformers':
    model_name = config['dataset'].get('transformer_model', 'unknown_model').replace("/", "_")
    save_dir_name = f"transformer_{model_name}_e{epochs}bs{batch_size}lr{lr}"
else:
    save_dir_name = f"experiment_e{epochs}bs{batch_size}lr{lr}"
